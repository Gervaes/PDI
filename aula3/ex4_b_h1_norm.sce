//CÁLCULO DE REALCE E AGUÇAMENTO: MÁSCARA h1 NORMALIZADA
...
//cálculo do detector de altas frequências (h1 normalizado)
for i=1:x
    for j=1:y
        if x-i >= 2 & y-j >= 2 then
            for m=i:i+2
            ...
            img1(i+1,j+1) = round((abs(soma))/5);
        end
        soma = 0;
    end
end

printf("MATRIZ H1 NORMALIZADA:\n");
for i=1:x
    for j=1:y
        printf("[%d] ", img1(i,j));
    end
